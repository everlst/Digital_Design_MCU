...
// ============================================================================
 module controller (
  ...
  // 内部连线
  // --------------------------------------------------------------------
 wire [1:0] FlagW;  // 条件满足时，哪些 NZCV 位需要被 datapath 写回wire [1:0] FlagW;  // 条件满足时，哪些 NZCV 位需要被 datapath 写回
 wire       PCS;  // 条件化 PC 写使能（分支指令）wire       PCS;  // 条件化 PC 写使能（分支指令）
 wire       RegW;  // 条件化寄存器写使能 (decoder → condlogic)wire       RegW;  // 条件化寄存器写使能 (decoder → condlogic)
 wire       MemW;  // 条件化数据存储器写使能 (decoder → condlogic)wire       MemW;  // 条件化数据存储器写使能 (decoder → condlogic)
