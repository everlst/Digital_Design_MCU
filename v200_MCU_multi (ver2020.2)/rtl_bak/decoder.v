...
// =============================================================================
(* dont_touch = "true" *) module decoder (
  input [1:0] Op,     // 指令主操作码
  ...
  // 内部连线 / 寄存器
  // --------------------------------------------------------------------
  (* dont_touch = "true" *) reg [12:0] controls;  // 译码表输出的 13 位组合控制信号

  // 由 controls 拆分出的单独控制线。wire 表示纯组合逻辑。
(* dont_touch = "true" *) wire Branch;  // 分支标志 (B / BL 指令)
(* dont_touch = "true" *) wire ALUOp;  // 1=数据处理指令，需要进一步解析 Funct[4:1] 来确定 ALU 运算
