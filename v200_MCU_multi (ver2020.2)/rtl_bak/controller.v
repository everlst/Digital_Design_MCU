...
// ============================================================================
(* dont_touch = "true" *) module controller (
  input         clk,       // 时钟
  ...
  // 内部连线
  // --------------------------------------------------------------------
(* dont_touch = "true" *) wire [1:0] FlagW;  // 条件满足时，哪些 NZCV 位需要被 datapath 写回wire [1:0] FlagW;  // 条件满足时，哪些 NZCV 位需要被 datapath 写回
(* dont_touch = "true" *) wire       PCS;  // 条件化 PC 写使能（分支指令）wire       PCS;  // 条件化 PC 写使能（分支指令）
(* dont_touch = "true" *) wire       RegW;  // 条件化寄存器写使能 (decoder → condlogic)wire       RegW;  // 条件化寄存器写使能 (decoder → condlogic)
(* dont_touch = "true" *) wire       MemW;  // 条件化数据存储器写使能 (decoder → condlogic)wire       MemW;  // 条件化数据存储器写使能 (decoder → condlogic)
