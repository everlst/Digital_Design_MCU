...
module decoder (
  ...
  // 内部连线 / 寄存器
  // --------------------------------------------------------------------
  reg  [10:0] controls;  // 译码表输出的 10 位组合控制信号
    ...
    casex (Op)
      2'b00: begin  // 数据处理 / 移位 指令
        if (Funct[5]) begin  // Funct[5] == 1 → 立即数格式
          ...
          if (Funct[4:1] === 4'b1101) begin
            controls = 11'b00_011_101000;
          end else begin
            controls = 11'b00_000_101001;
          end
        end else begin
          if (Funct[4:1] === 4'b1101) begin  // MOV 指令，寄存器
            controls = 11'b00_100_001000;  // MOV 指令，寄存器格式
          end else begin
            controls = 11'b00_000_001001;  // 寄存器格式
          end
        end
      end
      ...
      2'b01: begin  // 载入 / 存储 指令
        if (Funct[0])  // Funct[0] == 1 → LDR (Load)
          ...
          controls = 11'b10001110100;
      end

      2'b10:  // 分支 (B / BL)
      controls = 11'b01010100010;
