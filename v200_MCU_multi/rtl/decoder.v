...
(*DONT_TOUCH="YES"*) module decoder (
  ...
  // 内部连线 / 寄存器
  // --------------------------------------------------------------------
 (*DONT_TOUCH="YES"*) reg [12:0] controls;  // 译码表输出的 13 位组合控制信号

  // 由 controls 拆分出的单独控制线。wire 表示纯组合逻辑。
 (*DONT_TOUCH="YES"*) wire Branch;  // 分支标志 (B / BL 指令)
 (*DONT_TOUCH="YES"*) wire ALUOp;  // 1=数据处理指令，需要进一步解析 Funct[4:1] 来确定 ALU 运算
